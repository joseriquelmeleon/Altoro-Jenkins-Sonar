...
public class Utils {
	...
	public static void infoTestCase(String funcionality, String description) {
		...
		ClaseBase.logger.log(Status.INFO, m);
	...
	public static void eventFailed(String currentEvent, String errorMessage) {
		ClaseBase.logger.warning("No se pudo terminar el evento '" + currentEvent + "' a causa de: " + errorMessage);
		...
		ClaseBase.logger.log(Status.FAIL, m);
		String path = takeScreenshot(currentEvent);
		try {
			ClaseBase.logger.addScreenCaptureFromPath(path, currentEvent);
		} catch (IOException e) {
			e.printStackTrace();
		}
		Assert.fail();
	...
	public static String takeScreenshot(String nameMethod) {
		...
		File f = ((TakesScreenshot) ClaseBase.driver).getScreenshotAs(OutputType.FILE);
